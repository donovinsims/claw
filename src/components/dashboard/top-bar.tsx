...
export function TopBar({ isDark, toggleTheme, onStandupOpen, onCalendarOpen, onSearchOpen }: TopBarProps) {
  ...
  return (
    <header className="flex h-[68px] shrink-0 items-center justify-between border-b border-mc-border bg-surface px-3 sm:px-4 md:px-6">
      {/* Left */}
      <div className="flex items-center gap-2 md:gap-3">
        <Diamond className="hidden h-5 w-5 text-text-primary sm:block" />
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 md:gap-2.5">
        <button
          ...
          className="flex min-h-11 items-center gap-2 rounded-[var(--radius-inner)] bg-surface-elevated px-3 text-xs text-text-secondary transition-colors hover:text-text-primary"
        >
          <Search className="h-3.5 w-3.5" />
          <span>Search</span>
          <kbd className="hidden rounded border border-mc-border bg-surface px-1 py-0.5 font-mono text-[9px] lg:inline">⌘K</kbd>
        </button>
        <button
          onClick={onCalendarOpen}
          className="flex h-11 w-11 items-center justify-center rounded-[var(--radius-inner)] bg-surface-elevated text-text-secondary transition-colors hover:text-text-primary"
        >
          <Calendar className="h-3.5 w-3.5" />
        </button>
        <button
          onClick={onStandupOpen}
          className="flex min-h-11 items-center justify-center rounded-[var(--radius-inner)] bg-surface-elevated px-3 text-xs font-medium text-text-secondary transition-colors hover:text-text-primary"
        >
        ...
        <div className="hidden lg:flex items-center gap-1.5 rounded-[var(--radius-inner)] bg-[#34C759]/15 px-3 py-1">
